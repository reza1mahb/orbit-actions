...
import { NitroContracts1Point2Point1UpgradeAction, ProxyAdmin } from '../../../../contracts/parent-chain/contract-upgrades/NitroContracts1Point2Point1UpgradeAction.sol';
...
contract ExecuteNitroContracts1Point2Point1UpgradeScript is Script {
  function run() public {
    ...
    NitroContracts1Point2Point1UpgradeAction upgradeAction = NitroContracts1Point2Point1UpgradeAction(
        vm.envAddress('UPGRADE_ACTION_ADDRESS')
      );
    require(
      upgradeAction.newWasmModuleRoot() == wasmModuleRoot,
      'WASM_MODULE_ROOT mismatch'
    );
    require(
    ...
    require(
      isFeeTokenChain ==
        IIsUsingFeeToken(upgradeAction.newSequencerInboxImpl())
          .isUsingFeeToken(),
    ...
    bytes memory upgradeCalldata = abi.encodeCall(
      NitroContracts1Point2Point1UpgradeAction.perform,
      (rollup, proxyAdmin)
    );
    ...
    require(
      ...
      'ArbOS20Action: wasm module root not set'
    );
